...
- **Content is Markdown, rendered through a custom pipeline.** `Post.content` holds **Markdown**; `Post.content_html` (property) renders it via [kioblog/markdown/render.py](kioblog/markdown/render.py) and templates use `{{ post.content_html|safe }}`. The pipeline (`render_markdown`) wires a custom `CodeChromeExtension` plus Python-Markdown's `toc`. `Post.toc` exposes the generated table of contents (from `<h2>` headings) for the "On this page" index; `Post.reading_time` estimates minutes at ~200 wpm. `Post._render()` caches the result in `django.core.cache` (30-day timeout), keyed on a hash of `content` itself - a shared, cross-request cache, not just per-instance memoization - plus a same-instance memo so a second read within one request/instance doesn't even hit that cache. `Post.first_paragraph()` still exists (regex over HTML) but cards use `Post.excerpt`.
...
- **Models** ([models.py](kioblog/models.py)): `Category` (with `post_count()`), `Tag`, `Post` (ordered `-published, -id`; M2M `tags`, `excerpt`, `is_featured`, `published`; `get_previous`/`get_next`/`related_posts` helpers), and `Meta` (arbitrary key/value settings surfaced via the context processor). `models.py` imports `render_markdown` at module load, so the `kioblog.markdown` package must import cleanly for migrations to run.

  There used to be a `Comment` model too (self-referential `parent` for threading, created in `0001`) - removed in `0011_delete_comment` because it had no view, URL, form or template anywhere in the codebase. If comments are wanted, build the form/view/moderation pipeline properly rather than reintroducing a bare model nothing serves.
