...
from kioblog import decorators, models
...
class SearchView(generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if query:
            posts = models.Post.objects.filter(draft=False).filter(
                Q(title__icontains=query) | Q(excerpt__icontains=query) | Q(content__icontains=query)
            )
        return {"query": query, "posts": posts, "count": posts.count(), "recent_posts": models.Post.get_recent_posts()}
